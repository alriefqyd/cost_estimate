    ...
    <div class="table-responsive table-striped">
        <table class="table table-bordered">
            ...
            @foreach($estimateAllDisciplines as $key => $value)
                @php ($totalByWorkElement = $workItemController->sumTotalByWorkElement($value)['totalWorkCostByElement'])
                ...
                <tr style="background-color: #c4bd97">
                    ...
                    <td></td>
                    <td></td>
                    <td>{{$workItemController->sumTotalByWorkElement($value)['totalLaborByWorkElement']}}</td>
                    <td></td>
                    <td>{{$workItemController->sumTotalByWorkElement($value)['totalEquipmentByWorkElement']}}</td>
                    <td></td>
                    <td>{{$workItemController->sumTotalByWorkElement($value)['totalMaterialByWorkElement']}}</td>
            ...
            <tr class="bg-brown font-weight-bold">
                ...
                <td colspan="4">{{$workItemController->toCurrency($totalCost)}}</td>
            </tr>
            </tbody>
        </table>
    </div>
